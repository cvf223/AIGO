...
export class LegendarySyndicateSystem extends EventEmitter {
    ...
    async establishEliteSystemConnections() {
        ...
        if (this.eliteSystems.enhancedMemoryRewards?.serviceRegistry && this.eliteSystems.eliteJudge) {
            this.eliteSystems.enhancedMemoryRewards.serviceRegistry.eliteJudgeGatekeeper = this.eliteSystems.eliteJudge;
        }

        // CRITICAL FIX: Add null safety checks for system connections
        
        // Risk Management connects to Circuit Breakers
        if (this.eliteSystems.riskManagement && this.eliteSystems.circuitBreakers) {
            this.eliteSystems.riskManagement.circuitBreakers = this.eliteSystems.circuitBreakers;
        }

        // Smart Contract Evolution connects to Elite Judge for validation (if available)
        if (this.eliteSystems.smartContractEvolution && this.eliteSystems.eliteJudge) {
            this.eliteSystems.smartContractEvolution.judgeService = this.eliteSystems.eliteJudge;
        } else if (!this.eliteSystems.smartContractEvolution) {
            console.log('   ⚠️ SmartContractEvolutionSystem not available in construction mode');
        }

        // Production Monitoring connects to all critical systems (if available)
        ...
        // Elite Context Optimization connects to Local LLM (if both available)
        if (this.eliteSystems.eliteContextOptimization && this.eliteSystems.localOllamaLLM) {
            this.eliteSystems.eliteContextOptimization.localLLM = this.eliteSystems.localOllamaLLM;
        }
        
        // Deep Research connects to Knowledge Integrator (if both available)
        if (this.eliteSystems.deepResearch && this.eliteSystems.knowledgeIntegrator) {
