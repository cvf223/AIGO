  ...
  async initializeQuantumLearningProactivePreventionIntegration() {
    ...
    try {
      ...
      // Initialize quantum learning SFT governor
            // Import and initialize quantum learning SFT governor
      ...
      try {
        const CognitiveLoopModule = await import('../src/prevention/ProactiveCognitiveMetabolicLoop.js').catch(() => null);
        if (CognitiveLoopModule) {
          this.quantumLearningCognitiveMetabolicLoop = new CognitiveLoopModule.default({
          ...
          // Ensure the required method exists (preventing errors)
          if (!this.quantumLearningCognitiveMetabolicLoop.initializeConstructionSpecialistMetabolism) {
            ...
            this.quantumLearningCognitiveMetabolicLoop.initializeConstructionSpecialistMetabolism = async function() {
              console.log('🧠🏗️ [FALLBACK] Initializing construction specialist metabolism...');
              ...
              // Add default construction specialists
              const defaultSpecialists = ['architect', 'structural-engineer', 'cost-estimator', 'project-manager', 'site-supervisor'];
          ...
          console.warn('⚠️ ProactiveCognitiveMetabolicLoop module not found, creating fallback implementation');
          // Create minimal fallback implementation
          this.quantumLearningCognitiveMetabolicLoop = {
            agentId: 'quantum-learning-cognitive-fallback',
            enablePersistence: true,
            quantumLearningMode: true,
            orchestrateQuantumLearningImmunity: true,
            initialize: async () => true,
            initializeConstructionSpecialistMetabolism: async () => true,
            startCognitiveMetabolicLoop: async () => 'fallback_loop_id',
            transitionCognitivePhase: () => true,
            completeCognitiveLoop: () => ({ efficiency: 1.0, duration: 0 }),
            shutdown: async () => true
          };
        }
      } catch (cognitiveError) {
        console.warn('⚠️ Failed to initialize cognitive metabolic loop:', cognitiveError.message);
        // Create minimal fallback implementation
        this.quantumLearningCognitiveMetabolicLoop = {
          initialize: async () => true,
          initializeConstructionSpecialistMetabolism: async () => true
