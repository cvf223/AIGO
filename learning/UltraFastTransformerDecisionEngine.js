...
export class UltraFastTransformerDecisionEngine extends EventEmitter {
        ...
        return (strategy, performance) => {
            ...
            if (performance.dataSource !== 'REAL_BLOCKCHAIN_EXECUTION' ||
                ...
                if (this.learningPhaseMetrics) {
                    ...
                    // Only log occasionally to avoid spam - BUT SKIP IN OBSERVATION MODE
    ...
    async evolveDecisionStrategies() {
        try {
            ...
            if (!global.OBSERVATION_MODE_GLOBAL && !global.OBSERVATION_MODE_ENFORCED) console.log(`🧬 Evolution generation ${this.evolutionaryStrategy.generation}: Best fitness = ${elite[0]?.fitness?.toFixed(4) || 0}`);
    ...
    async shutdown() {
        ...
        if (this.state.modelLoaded) {
            try {
                ...
                console.warn('⚠️ Error saving model during shutdown:', error.message);
                // Try fallback method
                try {
                    await this.saveModelAndState();
                } catch (fallbackError) {
