    ...
    async getDatabaseConfig() {
        ...
        if (process.env.DATABASE_URL && process.env.DATABASE_URL !== 'mock://database') {
            ...
            try {
                ...
                console.log(`   🔍 DEBUG: Password contains special chars: ${/[!@#$%^&*(),.?":{}|<>]/.test(password)}`);
                
                // If password has special chars, ensure it's properly encoded
                if (/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
                    url.password = encodeURIComponent(password);
                    const fixedUrl = url.toString();
                    console.log('   🔧 Password has special characters - properly encoded');
                    
                    // Update the config with fixed URL
                    const { PRODUCTION_DB_CONFIG } = await import('../config/ProductionDatabaseConfig.js');
                    PRODUCTION_DB_CONFIG.connectionString = fixedUrl;
        ...
        const database = (process.env.POSTGRES_DB || 'construction_syndicate').trim();
