...
import { ConstructionComplexityMonitor, CONSTRUCTION_COMPLEXITY_THRESHOLDS as TRADING_COMPLEXITY_THRESHOLDS } from '../construction/safety/cognitive/ConstructionComplexityMonitor.js';
...
export class LLMJudgeCentralNervousSystem extends EventEmitter {
    ...
    async initializeSafetySystems() {
        ...
        try {
            // 🏗️ INITIALIZE ELITE CONSTRUCTION COMPLEXITY MONITOR
            console.log('     🏗️ Initializing Elite Construction Plan Complexity Monitor...');
            
            this.safetySystems.constructionComplexityMonitor = new ConstructionComplexityMonitor({
                enableRealTimeMonitoring: true,
                enableComplexityAlerts: true,
                enableThresholdWarnings: true,
                
                // Construction-specific configuration
                hoaiComplianceWeight: 0.25,
                structuralComplexityWeight: 0.30,
                coordinationComplexityWeight: 0.20,
                environmentalComplexityWeight: 0.15,
                safetyComplexityWeight: 0.10,
                
                // Advanced features
                enablePredictiveComplexity: true,
                enableComplexityRecommendations: true
            });
            
            await this.safetySystems.constructionComplexityMonitor.initialize();
            
            // 🎧 CONNECT CONSTRUCTION COMPLEXITY EVENTS (PROPER EventEmitter)
            this.safetySystems.constructionComplexityMonitor.on('complexityThresholdExceeded', (data) => {
                this.handleConstructionComplexityThreshold(data);
            });
            
            this.safetySystems.constructionComplexityMonitor.on('extremeComplexityDetected', (data) => {
                this.handleExtremeConstructionComplexity(data);
            });
            
            this.safetySystems.constructionComplexityMonitor.on('criticalComplexityDetected', (data) => {
                this.handleCriticalConstructionComplexity(data);
            });
            
            this.safetySystems.constructionComplexityMonitor.on('complexityAssessed', (data) => {
                this.processConstructionComplexityAssessment(data);
            });
            
            this.safetySystems.cognitiveCliffProtection = true;
            this.safetySystems.emergencyProtocols = true;
            this.safetySystems.constructionComplexityAnalysis = true;
            
            console.log('     ✅ Elite Construction Complexity Monitor integrated with central nervous system');
            console.log('     🎯 Advanced construction plan analysis capabilities activated');
