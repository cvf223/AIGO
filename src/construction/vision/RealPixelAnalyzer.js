...
export default class RealPixelAnalyzer extends EventEmitter {
    ...
    async detectAndClassifyElements(tiles, scale) {
        ...
        for (const tile of tiles) {
            ...
            const edges = await this.applyEdgeDetection(tile.image);
            ...
            for (const contour of contours) {
                const element = await this.analyzeContour(contour, tile, scale);
                
                if (element) {
                    allElements.push(element);
                    
                    if (element.classification !== 'undefined' && 
                        element.classification !== 'unclear' && 
                        element.classification !== 'irrelevant') {
                        classifiedElements.push(element);
                    } else {
                        unclassifiedElements.push(element);
                    }
                }
            }
        }
        ...
        classifiedElements.forEach(elem => {
            if (!this.analysisState.elementsDetected.has(elem.classification)) {
                this.analysisState.elementsDetected.set(elem.classification, []);
            }
            this.analysisState.elementsDetected.get(elem.classification).push(elem);
        });
        
        this.analysisState.unclassifiedElements = unclassifiedElements;
        
        return {
            all: allElements,
            classified: classifiedElements,
            unclassified: unclassifiedElements
        };
    }
