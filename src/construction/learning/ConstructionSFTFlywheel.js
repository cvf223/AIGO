...
export class ConstructionSFTFlywheel extends EventEmitter {
    ...
    async initialize() {
        ...
        try {
            ...
            if (this.config.enableVisionIntegration) {
                try {
                    // CRITICAL FIX: Pass ollamaService to prevent duplicate instances
    ...
    async generateSFTData(constructionScenario) {
        ...
        if (global.OBSERVATION_MODE_GLOBAL || global.OBSERVATION_MODE_ENFORCED) {
            return null; // Don't generate in observation mode
        }
    ...
    async generateExpertAnalysis(scenario) {
        ...
        // FIX: Use correct method (prompt instead of generateResponse)
        const expertResponse = await ollamaIntegration.prompt(
            expertPrompt,
            {
    ...
    async generateAverageAnalysis(scenario) {
        ...
        // FIX: Use correct method (prompt instead of generateResponse)
        const averageResponse = await ollamaIntegration.prompt(
            averagePrompt,
            {
    ...
    async evaluateAnalyses(scenario, expertAnalysis, averageAnalysis) {
        ...
        // FIX: Use correct method (prompt instead of generateResponse)
        const judgeEvaluation = await ollamaIntegration.prompt(
            judgePrompt,
            {
    ...
    startContinuousLearning() {
        ...
        // Generate synthetic scenarios periodically - BUT ONLY IF NOT IN OBSERVATION MODE
        this.scenarioInterval = setInterval(async () => {
            // CRITICAL: Skip in observation mode
            if (global.OBSERVATION_MODE_GLOBAL || global.OBSERVATION_MODE_ENFORCED) {
                return; // Don't generate scenarios in observation mode
            }
