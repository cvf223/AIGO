...
export class QuantumEntanglementEngine extends EventEmitter {
    constructor(config = {}) {
        ...
        if (global.OBSERVATION_MODE_ENFORCED || global.SKIP_ALL_SERVICES || global.MINIMAL_MODE) {
            console.log('🔭 QuantumEntanglementEngine: Disabled in observation mode');
            this.disabled = true;
            this.name = 'QuantumEntanglementEngine_Disabled';
    ...
    async findTrajectoryCooccurrences() {
        ...
        // CRITICAL FIX: Use actual agent_action_history schema (no trajectory_id column)
        try {
            ...
            if (trajectories.rows.length === 0) {
                console.log('   ⚠️ No successful agent trajectories found for entanglement analysis');
                return candidates; // Return empty array
            }
        } catch (sqlError) {
            console.warn('⚠️ Trajectory query failed:', sqlError.message);
            return candidates; // Return empty on SQL error
        }
    ...
    startEntanglementScanner() {
        // CRITICAL: Don't start in observation mode
        if (global.OBSERVATION_MODE_ENFORCED || global.OBSERVATION_MODE_GLOBAL) {
            console.log('⚠️ Skipping entanglement scanner - system in observation mode');
            return;
        }
        
        this.scanInterval = setInterval(async () => {
            // CRITICAL: Stop if observation mode is activated
            if (global.OBSERVATION_MODE_ENFORCED || global.OBSERVATION_MODE_GLOBAL || this.observationMode) {
                clearInterval(this.scanInterval);
                this.scanInterval = null;
                console.log('🛑 Stopping entanglement scanner - observation mode activated');
                return;
            }
    ...
    /**
     * Enter observation mode - stop all scanning
     */
    enterObservationMode() {
        console.log('🔭 QuantumEntanglementEngine entering observation mode');
     ...
     */
    exitObservationMode() {
        console.log('🔄 QuantumEntanglementEngine exiting observation mode');
        this.observationMode = false;
        // Note: Scanner must be manually restarted
    }
