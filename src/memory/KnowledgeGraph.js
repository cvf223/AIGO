...
export class KnowledgeGraph extends EventEmitter {
        ...
        if (this.db && typeof this.db.query === 'function') {
            try {
                ...
                if (!kg_nodes_exists || !kg_relationships_exists || !kg_entanglements_exists) {
                    console.warn('   ⚠️ CREATING MISSING TABLES: Knowledge Graph tables not found, creating...');
                    await this.createDatabaseTables();
                    console.log('   ✅ Knowledge Graph tables created successfully');
                } else {
                    console.log('   ✅ Knowledge Graph database tables verified');
                }
            } catch (error) {
    ...
    async createNode(nodeData) {
        ...
        if (this.useInMemoryFallback) {
            ...
            console.warn('   🧠 Using sophisticated in-memory quantum knowledge management');
        ...
        if (this.db && !this.useInMemoryFallback && !this.isShuttingDown) {
            try {
                additionalStats = await this.db.query(`
                    SELECT 
                        (SELECT AVG(n.confidence_score) FROM kg_nodes n) as avg_confidence,
                        COUNT(DISTINCT n.node_type) as unique_node_types,
                        COUNT(DISTINCT r.relationship_type) as unique_relationship_types
                    FROM kg_nodes n, kg_relationships r
                `);
            } catch (error) {
     ...
     * ========================
     * Creates the required database tables for knowledge graph persistence
     */
    async createDatabaseTables() {
        if (!this.db || typeof this.db.query !== 'function') {
            throw new Error('Database connection required to create tables');
        }
        
        try {
            console.log('   🏗️ Creating Knowledge Graph database schema...');
            ...
            console.log('   ✅ Knowledge Graph database schema created successfully');
            
        } catch (error) {
            console.error('   ❌ Failed to create Knowledge Graph database schema:', error.message);
            throw error;
        }
    }
}
