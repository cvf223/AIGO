...
export class SharedMemorySystem extends EventEmitter {
    constructor(databaseConfig) {
        ...
        // 🛡️ SAFE DATABASE INITIALIZATION: Handle both Pool objects and config objects
        if (databaseConfig && typeof databaseConfig.connect === 'function') {
            // Already a Pool object - use it directly
            this.database = databaseConfig;
            console.log('   🔗 Using existing database pool');
        } else if (databaseConfig && typeof databaseConfig === 'object') {
            // Configuration object - create new Pool
            this.database = new Pool(databaseConfig);
            console.log('   🔗 Created new database pool from config');
        } else {
            throw new Error('❌ SharedMemorySystem requires either a Pool object or database configuration');
        }
    ...
    /**
     * 📝 Ensure database tables exist with proper schema migration
     */
    async ensureTablesExist() {
        try {
            // First, check if table exists and what schema it has
            ...
            const schemaResult = await this.database.query(schemaCheckQuery);
            const existingColumns = schemaResult.rows.map(row => row.column_name);
            
            // Check if we have the old schema (key/value based)
            const hasOldSchema = existingColumns.includes('key') && existingColumns.includes('value');
            const hasNewSchema = existingColumns.includes('type') && existingColumns.includes('source');
            
            if (hasOldSchema && !hasNewSchema) {
                console.log('   🔄 Migrating SharedMemorySystem from old schema to new schema...');
                
                // Backup existing data
                const backupData = await this.database.query('SELECT * FROM shared_memory');
                console.log(`   💾 Backing up ${backupData.rows.length} existing records`);
                
                // Drop old table
                await this.database.query('DROP TABLE IF EXISTS shared_memory CASCADE');
                console.log('   🗑️ Dropped old table with incompatible schema');
            }
            ...
            console.log('   ✅ SharedMemorySystem table ensured with correct schema');
            ...
            console.log('   ✅ SharedMemorySystem indexes created successfully');
            
        } catch (error) {
            console.error('❌ SharedMemorySystem table creation failed:', error.message);
            throw error;
        }
    }
