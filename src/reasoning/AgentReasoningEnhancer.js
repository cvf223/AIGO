        ...
        agent.makeDecisionWithReasoning = async (context) => {
            ...
            try {
                ...
                if (this.isComplexDecision(context)) {
                    console.log('🌳 GRAPH-OF-THOUGHT REASONING INITIATED');
                    console.log('=====================================');
                    console.log(`🎯 Agent: ${agent.id || 'Unknown'}`);
                    console.log(`🎯 Context: ${JSON.stringify(context, null, 2).substring(0, 200)}...`);
                    console.log(`🧠 Reasoning Depth: ${this.config.maxReasoningDepth}`);
                    
                    explorationResults = await this.performTreeOfThought(agent, context, reasoningTrace);
                    
                    if (explorationResults) {
                        console.log('✅ GRAPH-OF-THOUGHT RESULTS:');
                        console.log(`   🌳 Thoughts explored: ${explorationResults.thoughtCount || 0}`);
                        console.log(`   🔗 Connections found: ${explorationResults.connectionCount || 0}`);
                        console.log(`   🎯 Best path confidence: ${explorationResults.bestPathConfidence || 'N/A'}`);
                        console.log(`   📊 Exploration efficiency: ${explorationResults.efficiency || 'N/A'}`);
                    }
                }
