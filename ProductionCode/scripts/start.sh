...
setup_environment() {
    ...
    # Only use the root .env file, do not copy or create from any other file
    if [ ! -f .env ]; then
        log_error "No .env file found in the root directory. Please ensure you have a .env file before running this script."
        exit 1
    fi
    log_info "Environment file already exists and will be used."
}
...
main() {
    ...
    if [ "$REINSTALL" = true ]; then
        log_info "Reinstalling: cleaning, installing, and building project as root (sudo)..."
        if ! sudo pnpm clean; then
            log_error "Failed to clean project (sudo)"
            exit 1
        fi
        log_success "Project cleaned (sudo)"
        if ! sudo pnpm install --no-frozen-lockfile; then
            log_error "Failed to install dependencies (sudo)"
            exit 1
        fi
        log_success "Dependencies installed (sudo)"
        if ! sudo pnpm build; then
            log_error "Failed to build project (sudo)"
            exit 1
        fi
        log_success "Project built successfully (sudo)"
    fi
