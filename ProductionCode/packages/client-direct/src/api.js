...
function validateUUIDParams(params, res) {
    const agentId = (0, core_2.validateUuid)(params.agentId);
    if (!agentId) {
        res.status(400).json({
    ...
    if (params.roomId) {
        const roomId = (0, core_2.validateUuid)(params.roomId);
        if (!roomId) {
            res.status(400).json({
...
function createApiRouter(agents, directClient) {
    ...
    router.use(express_1.default.json({
        limit: (0, core_1.getEnvVariable)("EXPRESS_MAX_PAYLOAD") || "100kb",
    }));
    router.get("/", (req, res) => {
    ...
    router.get('/storage', async (req, res) => {
        try {
            const uploadDir = path_1.default.join(process.cwd(), "data", "characters");
    ...
    router.post("/agents/:agentId/set", async (req, res) => {
        ...
        try {
            (0, core_1.validateCharacterConfig)(character);
        }
        catch (e) {
            core_1.console.error(`Error parsing character: ${e}`);
            res.status(400).json({
                success: false,
                message: e.message,
            });
        ...
        try {
            ...
            core_1.console.log(`${character.name} started`);
        }
        catch (e) {
            core_1.console.error(`Error starting agent: ${e}`);
            res.status(500).json({
                success: false,
                message: e.message,
            });
        ...
        if (process.env.USE_CHARACTER_STORAGE === "true") {
            try {
                ...
                await fs_1.default.promises.writeFile(filepath, JSON.stringify({ ...characterJson, id: agent.agentId }, null, 2));
                core_1.console.info(`Character stored successfully at ${filepath}`);
            }
            catch (error) {
                core_1.console.error(`Failed to store character: ${error.message}`);
            }
        }
        res.json({
            id: character.id,
    ...
    //     } catch (error) {
    //         console.error("Failed to get TEE agents:", error);
    //         res.status(500).json({
    ...
    //     } catch (error) {
    //         console.error("Failed to get TEE agent:", error);
    //         res.status(500).json({
    ...
    //             });
    //         } catch (error) {
    //             console.error("Failed to get TEE logs:", error);
    //             res.status(500).json({
