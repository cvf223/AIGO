...
import { evaluationTemplate, formatEvaluatorExamples, formatEvaluatorNames, formatEvaluators, } from "./evaluators";
...
// RAGKnowledgeItem,
//Media,
ModelClass, ModelProviderName, } from "./types";
...
export class AgentRuntime {
    ...
    constructor(opts) {
        ...
        this.evaluators.push(FactEvaluator);
...
Text: ${attachment.text}
    ...
    /**
     * Broadcast a high-priority fact/knowledge to the group chat for other agents to see and evaluate.
     */
    async sendGroupAnnouncement(fact) {
        const groupChatId = this.character.clientConfig?.telegram?.autoPost?.mainChannelId;
        if (!groupChatId)
            return;
        const text = `📢 *High-Priority Knowledge Shared!*\n\n*Fact:* ${fact.claim}\n*Proof:* ${fact.proof}\n*Related Task:* ${fact.relatedTask || "N/A"}\n*From Agent:* ${this.agentId}`;
        // Use your Telegram bot instance to send the message
        // Assumes the first client is the Telegram client
        const telegramClient = this.clients[0];
        if (telegramClient && telegramClient.bot && telegramClient.bot.telegram) {
            await telegramClient.bot.telegram.sendMessage(groupChatId, text, { parse_mode: "Markdown" });
        }
    }
}
...
async function storeConclusionMemory(runtime, content, roomId, userId) {
    const memory = {
        id: generateUuid(),
...
async function summarizeRecentMemories(runtime, roomId, count = 10) {
    const memories = await runtime.messageManager.getMemories({
        roomId,
        count,
    });
    // Replace this with your actual summarization logic/LLM call
    return memories.map(m => m.content.text).join(' ');
}
async function fetchRelevantKnowledge(runtime, query) {
    return await runtime.ragKnowledgeManager.getKnowledge({
        agentId: runtime.agentId,
        query,
        limit: 5,
    });
}
// Utility: Extract facts from recent messages (replace with your own logic/LLM)
function extractFactsFromMessages(messages) {
    // Example: Just return the last message as a "fact"
    return messages.slice(-1).map(m => ({
        claim: m.content.text,
        type: "fact"
    }));
}
export const FactEvaluator = {
    name: "FactEvaluator",
    description: "Extracts facts, statuses, and opinions from recent messages, only if supported by explicit evidence.",
    similes: ["fact", "reflection", "learning"],
    alwaysRun: false,
    examples: [],
    validate: async (runtime, message) => {
        const recent = await runtime.messageManager.getMemories({ roomId: message.roomId, count: 10 });
    ...
    handler: async (runtime, message, state, options, callback) => {
        ...
        const facts = await extractFactsWithLLM(runtime, recent, currentTasks);
        for (const fact of facts) {
            // Create memory with embedded proof
            const memoryContent = {
                text: fact.claim,
                type: fact.type,
                proof: fact.proof,
                priority: fact.priority || "normal",
                relatedTask: fact.relatedTask || null,
                evidence: {
                    source: fact.proof,
                    timestamp: Date.now(),
                    context: recent.map(m => m.content.text).join("\n"),
                    supportingMessages: recent.map(m => ({
                        id: m.id,
                        text: m.content.text,
            ...
            // If high priority, broadcast to group chat
            if (fact.priority === "high" || fact.priority === "highest") {
                await runtime.sendGroupAnnouncement(fact);
            }
        }
    }
};
async function extractFactsWithLLM(runtime, recentMessages, currentTasks) {
    const context = recentMessages.map(m => m.content.text).join("\n");
    const taskList = currentTasks.map(t => `- ${t.name || t.description || t}`).join("\n");
    // Fetch prior knowledge relevant to the conversation
    const priorKnowledgeItems = await runtime.ragKnowledgeManager.getKnowledge({
        agentId: runtime.agentId,
        query: context,
        limit: 8,
    });
    const priorKnowledge = priorKnowledgeItems.map(k => k.content.text).join("\n");
    const prompt = `
Extract all new facts, statuses, and opinions from the following conversation.
Only include items that are supported by explicit evidence (such as a link, quote, or referenced message).

Here are the agent's current tasks/objectives:
${taskList}

Here is what the agent already knows:
${priorKnowledge}

For each item, return a JSON object with:
- claim: the fact, status, or opinion
- type: one of 'fact', 'status', or 'opinion'
- proof: a URL, quote, or reference that supports the claim (must not be empty)
- relatedTask: the name/description of the related task if any, or null
- priority: 'high' if the claim is directly related to a current task, is novel, or is strongly supported by prior knowledge; otherwise 'normal'
- already_known: true/false (set to true if the claim is already in the prior knowledge)
- in_bio: true/false
- supporting_messages: array of message IDs that support this claim
...
${context}
`;
    const llmOutput = await generateText({
        runtime,
        context: prompt,
    ...
    let facts = [];
    try {
        facts = JSON.parse(llmOutput);
    }
    catch (e) {
        const match = llmOutput.match(/\[.*\]/s);
        if (match)
            facts = JSON.parse(match[0]);
    }
    // Only keep facts with a non-empty proof and not already known
    return facts.filter(f => f.proof && f.proof.trim() !== "" && !f.in_bio && f.claim && f.claim.trim() !== "");
}
