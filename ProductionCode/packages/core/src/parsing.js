...
export function parseJSONObjectFromText(text) {
    try {
        ...
        try {
            ...
            // Remove any "Available Actions" or template text that might have leaked
            text = text.replace(/# Available Actions[\s\S]*?(?={)/, '');
            // Remove any other template instructions that might have leaked
            text = text.replace(/# CRITICAL RESPONSE FORMAT INSTRUCTIONS[\s\S]*?(?={)/, '');
        ...
        elizaLogger.error("Error parsing JSON response:", error);
        elizaLogger.error("Original text:", text);
        throw error;
    }
}
/**
