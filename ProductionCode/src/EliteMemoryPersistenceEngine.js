    ...
    this.securityManager = {
      ...
      signMemory: (memoryData, keyId) => {
        ...
        if (memoryData === undefined || memoryData === null) {
          console.warn('⚠️ Attempting to sign undefined/null memory data. Creating empty memory signature.');
          memoryData = { 
            type: 'empty_memory',
            timestamp: Date.now(),
        ...
        try {
          ...
          if (!keyPair) {
            console.warn(`⚠️ Key pair not found: ${keyId}, using fallback key`);
  ...
  async storeQuantumMemory(agentId, memoryType, memoryData, options = {}) {
    ...
    try {
      ...
      if (!this.dbPool || typeof this.dbPool.connect !== 'function') {
        ...
        console.log(`💾 Stored in in-memory store: ${memoryId}`);
  ...
  async retrieveQuantumMemory(memoryId, options = {}) {
    ...
    try {
      ...
      try {
        ...
        console.log(`🧠 Memory retrieved: ${memoryId} (${accessTime}ms, fidelity: ${quantumState.fidelity.toFixed(3)})`);
