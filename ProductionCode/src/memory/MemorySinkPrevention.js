...
export class MemorySinkPrevention extends EventEmitter {
        ...
        const query = `
            WITH similarity_pairs AS (
                SELECT 
                    n1.node_id as node1,
                    n2.node_id as node2,
                    1 - (n1.concept_embedding <=> n2.concept_embedding) as similarity
                FROM kg_nodes n1
                CROSS JOIN kg_nodes n2
                WHERE n1.node_id < n2.node_id
                AND n1.node_type = n2.node_type
                AND 1 - (n1.concept_embedding <=> n2.concept_embedding) > 0.9
            )
            SELECT 
                COUNT(*) as redundant_pairs,
                AVG(similarity) as avg_similarity
            FROM similarity_pairs
        `;
        
        const result = await this.db.query(query);
        const totalNodes = await this.knowledgeGraph.metrics.nodeCount;
        
        // Calculate redundancy ratio
        const redundancy = result.rows[0].redundant_pairs / Math.max(1, totalNodes);
    ...
    async measureSystemPerformance() {
        ...
        // 🔥 FIX: Handle bigint timestamp vs PostgreSQL timestamp
        ...
        const query = `
            ...
            WHERE 
                (CASE 
                    WHEN timestamp IS NOT NULL AND timestamp > 1000000000 
                    THEN timestamp > ${oneHourAgo}
                    ELSE created_at > NOW() - INTERVAL '1 hour'
                END)
        `;
        ...
        return {
            score: (metrics.avg_performance + metrics.avg_success_rate + metrics.avg_efficiency) / 3,
            timestamp: Date.now(),
            components: metrics
        };
    }
