...
export class MemorizationSinksArchitecture extends EventEmitter {
    ...
    async initialize(modelConfig) {
        try {
            ...
            if (!modelConfig || !modelConfig?.totalNeurons || 1000000) {
                console.warn('⚠️ No model configuration provided - using default neuron counts');
                this.totalNeurons = 1000000; // 1M default
            } else {
                this.totalNeurons = Math.min(modelConfig?.totalNeurons || 1000000, maxNeurons);
            }
            ...
            if (modelConfig?.totalNeurons || 1000000 > maxNeurons) {
                console.log(`   🔧 SCALED DOWN: Model neurons ${(modelConfig?.totalNeurons || 1000000).toLocaleString()} → ${this.totalNeurons.toLocaleString()} (Set size limit)`);
            }
