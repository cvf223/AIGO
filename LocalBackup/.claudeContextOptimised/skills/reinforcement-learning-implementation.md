...
This skill provides a production-ready reinforcement learning implementation for construction optimization in the AIGO-Syndicate system. It includes PPO (Proximal Policy Optimization) algorithm, experience replay, reward engineering, and comprehensive training infrastructure.
...
```javascript
// construction-rl-integration.js
import { createRLEngine } from './reinforcement-learning-engine.js';
import { DatabasePoolManager } from '../core/DatabasePoolManager.js';

export class ConstructionRLService {
    constructor() {
    ...
    async initialize() {
        // Initialize different RL engines for different optimization tasks
        await this.initializeScheduleOptimizer();
        await this.initializeResourceAllocator();
        await this.initializeSafetyPredictor();
    }
    
    async initializeScheduleOptimizer() {
        const engine = createRLEngine({
            modelId: 'schedule-optimizer',
            stateSize: 50, // Project features
            actionSize: 20, // Scheduling decisions
            environmentType: 'schedule'
        });
        
        await engine.initialize();
        this.engines.set('schedule', engine);
    }
    
    async initializeResourceAllocator() {
        const engine = createRLEngine({
            modelId: 'resource-allocator',
            stateSize: 100, // Resource state
            actionSize: 50, // Allocation decisions
            environmentType: 'resources'
        });
        
        await engine.initialize();
        this.engines.set('resources', engine);
    }
    
    async initializeSafetyPredictor() {
        const engine = createRLEngine({
            modelId: 'safety-predictor',
            stateSize: 30, // Safety indicators
            actionSize: 10, // Safety measures
            environmentType: 'safety'
        });
        
        await engine.initialize();
        this.engines.set('safety', engine);
    }
    
    async optimizeProjectSchedule(projectId) {
        const engine = this.engines.get('schedule');
        
        // Load project data
        const project = await this.loadProject(projectId);
        
        // Create custom environment
        const environment = this.createScheduleEnvironment(project);
        ...
        let done = false;
        while (!done) {
            const { action } = await engine.selectAction(state, false);
            const { nextState, reward, isDone } = environment.step(action);
            
            schedule.push({
                action: environment.decodeScheduleAction(action),
                state: nextState,
                expectedOutcome: reward
            });
        ...
        return {
            ...
            expectedDuration: schedule.length,
            confidenceScore: await this.calculateConfidence(engine, environment)
        };
    }
    
    async trainOnHistoricalProjects() {
        // Load completed projects
        const projects = await this.loadCompletedProjects();
        
        for (const project of projects) {
            // Create environment from historical data
            const environment = this.createHistoricalEnvironment(project);
            
            // Train appropriate engine
            const engineType = this.determineEngineType(project);
            const engine = this.engines.get(engineType);
            
            if (engine) {
                await engine.trainEnvironment(environment, 100);
            }
        }
    }
    
    createScheduleEnvironment(project) {
        return {
            state: {
                tasks: project.tasks,
            ...
            step(action) {
                ...
                const task = this.state.tasks[action];
                
                if (this.canExecuteTask(task)) {
                    this.state.completedTasks.add(task.id);
                    this.state.currentDay += task.duration;
                }
                
                const reward = this.calculateScheduleReward();
                const done = this.state.completedTasks.size === this.state.tasks.length;
            ...
            getStateVector() {
                ...
                return [
                    this.state.currentDay / 365,
                    this.state.completedTasks.size / this.state.tasks.length,
                    // ... more features
                ];
            }
        };
    ...
    async loadCompletedProjects() {
        const result = await this.dbPool.query(`
            SELECT p.*, 
                   array_agg(pt.* ORDER BY pt.start_date) as tasks
            FROM projects p
...
describe('ReinforcementLearningEngine', () => {
    ...
    test('should select actions', async () => {
        const state = environment.reset();
        const { action, logProb, value } = await engine.selectAction(state);
    ...
    test('should collect rollout', async () => {
        const rollout = await engine.collectRollout(environment);
        
        expect(rollout.states).toHaveLength(engine.config.rolloutSteps);
        expect(rollout.actions).toHaveLength(engine.config.rolloutSteps);
        expect(rollout.rewards).toHaveLength(engine.config.rolloutSteps);
        expect(rollout.advantages).toBeDefined();
    });
    
    test('should train on rollout', async () => {
        const rollout = await engine.collectRollout(environment);
        const losses = await engine.train(rollout);
        
        expect(losses.policyLoss).toBeDefined();
        expect(losses.valueLoss).toBeDefined();
        expect(losses.entropy).toBeDefined();
    });
    
    test('should save and load checkpoints', async () => {
        await engine.saveCheckpoint('test_checkpoint');
        
        const newEngine = createRLEngine({
            modelId: engine.config.modelId,
            stateSize: 10,
            actionSize: 5
        });
        await newEngine.initialize();
        
        const loaded = await newEngine.loadCheckpoint('test_checkpoint');
        expect(loaded).toBe(true);
    });
});

describe('ConstructionEnvironment', () => {
    let env;
    
    beforeEach(() => {
        env = new ConstructionEnvironment();
    });
    
    test('should reset properly', () => {
        const state = env.reset();
        expect(state).toHaveLength(12);
        expect(env.state.day).toBe(0);
        expect(env.state.progress).toBe(0);
    });
    
    test('should step through environment', () => {
        env.reset();
        const { nextState, reward, done } = env.step(10);
        
        expect(nextState).toHaveLength(12);
        expect(typeof reward).toBe('number');
        expect(typeof done).toBe('boolean');
        expect(env.state.day).toBe(1);
    });
});
```

This implementation provides a complete reinforcement learning system with PPO algorithm, experience replay, and construction-specific environments for the AIGO-Syndicate system.
