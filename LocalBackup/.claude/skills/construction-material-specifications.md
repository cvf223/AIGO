# Construction Material Specifications Implementation

## Overview

This skill provides a production-ready material management system for the AIGO-Syndicate construction intelligence. It includes material database schemas, strength calculations, load distribution algorithms, safety factors, compatibility checking, and cost optimization with comprehensive monitoring.
...
```javascript
// construction-material-integration.js
import { createMaterialEngine } from './construction-material-engine.js';
import { DatabasePoolManager } from '../core/DatabasePoolManager.js';

export class ConstructionMaterialService {
    constructor() {
    ...
    async initialize() {
        this.materialEngine = createMaterialEngine({
            categories: ['concrete', 'steel', 'wood', 'masonry', 'insulation']
        });
        
        await this.materialEngine.initialize();
    ...
    setupEventHandlers() {
        this.materialEngine.on('error', (error) => {
            console.error('Material service error:', error);
        });
        
        this.materialEngine.on('metrics', (metrics) => {
            this.logMetrics(metrics);
        });
    }
    
    async validateStructuralDesign(designData) {
        const validations = [];
        
        // Validate each structural element
        ...
        const compatibilityIssues = await this.checkDesignCompatibility(designData);
        
        // Calculate total material requirements
        const materials = await this.calculateMaterialRequirements(designData);
        
        // Optimize if requested
        let optimization = null;
        if (designData.optimize) {
            optimization = await this.optimizeDesign(designData, materials);
        }
    ...
    async validateElement(element) {
        try {
            // Get material
            const material = await this.materialEngine.getMaterial(element.material);
            
            // Calculate loads
            const loads = this.calculateElementLoads(element);
            
            // Check strength
            const strength = await this.materialEngine.calculateStrength(
                element.material,
                element.loadType || 'compression',
                element.geometry
            );
            
            // Calculate utilization
            const utilization = loads.design / strength.capacity;
            ...
            return {
                ...
                designLoad: loads.design,
                capacity: strength.capacity,
                utilization,
                passed,
                calculation: strength.calculation
            };
    ...
    calculateElementLoads(element) {
        // Apply load factors
        const factors = this.materialEngine.config.loadFactors;
        
        let totalLoad = 0;
        
        if (element.loads) {
            element.loads.forEach(load => {
                const factor = factors[load.type] || 1.0;
        ...
        return {
            design: totalLoad,
            characteristic: totalLoad / 1.5
        };
    }
    
    async checkDesignCompatibility(designData) {
        const issues = [];
        const checkedPairs = new Set();
        
        // Check all material pairs in contact
        for (const connection of designData.connections || []) {
            const key = [connection.material1, connection.material2].sort().join(':');
            
            if (!checkedPairs.has(key)) {
                checkedPairs.add(key);
                
                const compatibility = await this.materialEngine.checkCompatibility(
                    connection.material1,
                    connection.material2,
                    connection.type || 'structural'
                );
                
                if (compatibility.compatibility_score < 0.8) {
                    issues.push({
                        materials: [connection.material1, connection.material2],
    ...
    async calculateMaterialRequirements(designData) {
        const requirements = {};
        
        for (const element of designData.elements) {
            const material = element.material;
            const volume = this.calculateElementVolume(element.geometry);
            ...
            requirements[material].quantity += volume;
            requirements[material].elements.push(element.id);
        }
        
        // Add waste factors
        for (const mat of Object.values(requirements)) {
            mat.quantityWithWaste = mat.quantity * 1.05; // 5% waste
        }
        
        return requirements;
    }
    
    calculateElementVolume(geometry) {
        if (geometry.volume) return geometry.volume;
        
        // Calculate based on geometry type
        if (geometry.type === 'rectangular') {
            return (geometry.width * geometry.height * geometry.length) / 1e9; // mm³ to m³
        } else if (geometry.type === 'circular') {
            return (Math.PI * Math.pow(geometry.diameter / 2, 2) * geometry.length) / 1e9;
        }
        
        return 0;
    }
    
    async optimizeDesign(designData, currentMaterials) {
        const optimizations = [];
        
        // Try to optimize each material category
        for (const [material, requirement] of Object.entries(currentMaterials)) {
            const currentMat = await this.materialEngine.getMaterial(material);
            
            const optimization = await this.materialEngine.optimizeMaterialSelection({
                category: currentMat.category,
                minStrength: this.getRequiredStrength(designData, material),
                environmental: designData.environmentalClass
            }, {
                budget: designData.budget,
                quantities: { [currentMat.category]: requirement.quantity }
            });
            
            if (optimization.totalCost < requirement.quantity * currentMat.unit_cost) {
                optimizations.push({
                    original: material,
                    suggested: optimization.selectedMaterials[0]?.material.material_code,
                    savings: requirement.quantity * currentMat.unit_cost - optimization.totalCost,
                    performance: optimization.performanceMetrics
                });
            }
        }
    ...
    getRequiredStrength(designData, material) {
        // Find minimum required strength for this material
        const elements = designData.elements.filter(e => e.material === material);
        let minStrength = 0;
        
        elements.forEach(element => {
            if (element.requiredStrength) {
                minStrength = Math.max(minStrength, element.requiredStrength);
            }
        });
        
        return minStrength || 25; // Default minimum
    }
    
    async generateMaterialReport(projectId) {
        // Generate comprehensive material report for project
        const report = {
            projectId,
            timestamp: new Date(),
            materials: [],
        ...
        // Get all materials used in project
        const materials = await this.getProjectMaterials(projectId);
        
        for (const mat of materials) {
            const material = await this.materialEngine.getMaterial(mat.material_code);
            
            const cost = mat.quantity * material.unit_cost;
            const weight = mat.quantity * material.density;
            const carbon = mat.quantity * (material.carbon_footprint || 0);
    ...
    async generateRecommendations(report) {
        ...
        if (report.carbonFootprint > 0) {
            const carbonNegativeMaterials = await this.findCarbonNegativeMaterials();
            if (carbonNegativeMaterials.length > 0) {
                recommendations.push({
                    type: 'sustainability',
                    message: 'Consider using carbon-negative materials like engineered wood products',
                    options: carbonNegativeMaterials
                });
            }
        }
        ...
        if (report.totalCost > 0) {
            const costSavings = await this.identifyCostSavings(report);
    ...
    async getProjectMaterials(projectId) {
        const result = await this.dbPool.query(`
            SELECT material_code, SUM(quantity) as quantity, unit
    ...
    async findCarbonNegativeMaterials() {
        const result = await this.dbPool.query(`
            SELECT material_code, name, carbon_footprint
            FROM material_specifications
            WHERE carbon_footprint < 0
    ...
    async identifyCostSavings(report) {
        const savings = [];
        
        // Check for bulk discounts
        for (const material of report.materials) {
            const availability = await this.materialEngine.checkMaterialAvailability(
                material.code,
                material.quantity * 1.5 // Check for 50% more
            );
            
            if (availability.bestOption && 
                availability.bestOption.unitPrice < material.unitCost) {
                savings.push({
                    material: material.code,
                    currentCost: material.unitCost,
                    bulkPrice: availability.bestOption.unitPrice,
                    potentialSaving: (material.unitCost - availability.bestOption.unitPrice) * material.quantity
                });
            }
        }
    ...
    async logMetrics(metrics) {
        ...
        console.log('Material Engine Metrics:', metrics);
    }
}
```
...
```javascript
// material-engine.test.js
import { describe, test, expect, beforeEach } from '@jest/globals';
import { createMaterialEngine } from './construction-material-engine.js';

describe('ConstructionMaterialEngine', () => {
    let engine;
    
    beforeEach(async () => {
        engine = createMaterialEngine({
            cacheSize: 100
        });
        await engine.initialize();
    });
    
    test('should load material specifications', async () => {
        const concrete = await engine.getMaterial('C30/37');
        
        expect(concrete).toBeDefined();
        expect(concrete.category).toBe('concrete');
        expect(concrete.properties.compressiveStrength).toBe(37);
    });
    
    test('should calculate concrete strength', async () => {
        const strength = await engine.calculateStrength('C30/37', 'compression', {
            width: 400,
            height: 400
        });
        
        expect(strength.designStrength).toBeLessThan(37); // Safety factor applied
        expect(strength.capacity).toBeGreaterThan(0);
        expect(strength.calculation).toContain('EC2');
    });
    
    test('should check material compatibility', async () => {
        const compatibility = await engine.checkCompatibility(
            'C30/37',
            'S355',
            'structural'
        );
        
        expect(compatibility.compatibility_score).toBeGreaterThan(0.5);
        expect(compatibility.compatibility_type).toBe('structural');
    });
    
    test('should optimize material selection', async () => {
        const result = await engine.optimizeMaterialSelection({
            category: 'concrete',
            minStrength: 25
        }, {
            budget: 10000,
            quantities: { concrete: 100 }
        });
        
        expect(result.selectedMaterials).toBeDefined();
        expect(result.totalCost).toBeGreaterThan(0);
        expect(result.performanceMetrics).toBeDefined();
    });
    
    test('should calculate load distribution', async () => {
        const loads = [
            { type: 'uniform', value: 10, length: 5000 }
        ];
        const supports = [
            { position: 0, type: 'pinned' },
            { position: 5000, type: 'roller' }
        ];
        
        const distribution = await engine.calculateLoadDistribution(
            'beam',
            loads,
            supports,
            null
        ...
        expect(distribution.reactions).toHaveLength(2);
        expect(distribution.moments).toBeDefined();
        expect(distribution.shears).toBeDefined();
    });
});
```

This implementation provides a comprehensive material management system with strength calculations, compatibility checking, cost optimization, and full integration with the construction syndicate system.
